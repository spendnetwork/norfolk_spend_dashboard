...
try {
	data2.forEach(function(d) {
		data3.push( new Object({ service: d.key,total_spend: d.values[0],year:d.values[1]}));
	jj++;
	if(jj<=20) {
	top10.push (new Object({ service : d.key,	total_spend : d.values[0],year: d.values[1]}));
	...
	var svg1_1b = dimple.newSvg("#r1b1 .graph_wrapper", "100%", "400");     	
        ...
        var y = barChart.addCategoryAxis("y", "service");
    ...
    barChartseries.afterDraw = function(s, d) {
        ...
        svg1_1b.append("text")
            ...
            .attr("y", parseFloat(shape.attr("y")) + 20) // need to calculated height based on number in series
            .style('fill', "black")
            .text(d.cy);
    	};
...
{	
    ...
    // create the table header
     d3.select("#data_table thead")
        ...
        .selectAll("th")
        .data(d3.keys(data[0]))
        .enter().append("th").text(function(d){ console.log(d);if(d== "service") return "Service"; if(d == "total_spend") return "Amount"; if(d == "YYYY") return "Year";  if(d == "date_calculated") return "Date"; });
